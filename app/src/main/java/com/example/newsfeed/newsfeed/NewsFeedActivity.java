...
public class NewsFeedActivity extends AppCompatActivity implements LoaderCallbacks<List<NewsFeed>> {
    ...
    // URL for News Listing data from the News dataset related to Android search
    private static String News_LIST_URL =
            "https://content.guardianapis.com/search?q=android&api-key=303c20ef-6d83-4974-960c-9777b50f0194";

    // Constant value for the NewsFeed loader ID
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ListView newslistListView = (ListView) findViewById(R.id.list);

        // First launches of empty status
        mEmptyStateTextView = (TextView) findViewById(R.id.empty_view);
        newslistListView.setEmptyView(mEmptyStateTextView);
        ...
        newslistListView.setAdapter(mAdapter);

        // Set an item click listener on the ListView, which sends an intent to a web browser
        // to open a website with more information about the selected newsfeed.
        newslistListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
                // Find the current newsfeed that was clicked on
                NewsFeed currentNewslist = mAdapter.getItem(position);

                // Convert the String URL into a URI object (to pass into the Intent constructor)
                Uri newslistUri = Uri.parse(currentNewslist.getUrl());
            ...
            public Loader<List<NewsFeed>> onCreateLoader(int i, Bundle bundle) {
                ...
                return new NewsFeedLoader(NewsFeedActivity.this, News_LIST_URL);
            }
        });
    ...
    public Loader<List<NewsFeed>> onCreateLoader(int i, Bundle bundle) {
        ...
        // Create a new loader for the given URL
        return new NewsFeedLoader(this, News_LIST_URL);
    }
    ...
    public void onLoadFinished(Loader<List<NewsFeed>> loader, List<NewsFeed> newsFeeds) {
        ...
        // If there is a valid list of {@link Newslist}s, then add them to the adapter's
